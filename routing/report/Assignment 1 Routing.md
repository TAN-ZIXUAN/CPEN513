...
Lee-Moore (or Dijkstra)  and A* are algorithm for finding shortest between nodes in a graph. Here we implement them on a 2d grid, routing sources and sinks.  The difference between the two algorithms are their cost functions. The cost function of Lee Moore only uses the cell’s distance to the start cell, while A* also calculates the estimate distance from the current cell to the target. A* algorithms searches neighbour cells towards the direction of the target and Lee-Moore algorithm just explores the surroundings equally. Thus A* is often faster than Lee-Moore. 

In the programme, we route one source first, letting it exploring the grid and connects the sink which it hits first. In this way, we form our first path. As the rest of the sinks can be connected to either the source or the constructed paths, we route from sinks and let it explore the grid for source  or the paths. When routing wires with just only one source and one sink, we use A* algorithm. For the exploring part, we use Lee-Moore algorithm as we do not have a specific target.
...
Routing orders may effect the routing results. Both Lee-Moore and A* are shortest path algorithms. Some wires might block others’ path. So routing order matters. We have tried different solutions for this problem.
...
To test the routing function with different benchmark files, `pparametrize` from `pytest` is used. We create a test_input, a list of benchmark file  and a list of expected output.
...
(*Here for impossible file, the expected output is the max amount of segments that can be routed rather than the total number of  segment. So the routing algorithm should be able to pass the tests for the two impossible benchmark files)
...
(*Here for impossible file, the expected output is the max amount of segments that can be routed rather than the total number of segments. So the routing algorithm should be able to pass the tests for the two impossible benchmark files)
...
The routed image results shown as below. (The gif animation is on GitHub)
